...
class Sondage(Cog):
    ...
    async def sondage(self, ctx: commands.Context, question, proposition1, proposition2):
        class MessagePoll(self, commands.Context, question, proposition1, proposition2):
            ...
            @staticmethod
            async def button_callback1(interaction, poll_id=poll_id):
            ...
            @staticmethod
            async def button_callback2(interaction, poll_id=poll_id):
                utilisateur = str(interaction.user)
                post = f"🟥 {utilisateur}"
                histo = await ctx.message.channel.history(limit=50).flatten()
                first_reaction = True
            ...
            async def poll_verif(interaction, poll_id=poll_id):
                utilisateur = str(interaction.user)

            async def poll_stats(interaction, poll_id=poll_id):
                utilisateur = str(interaction.user)

            button1.callback = button_callback1
            button2.callback = button_callback2

        poll_call = MessagePoll
